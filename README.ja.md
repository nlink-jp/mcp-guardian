...
- **転送不能リクエストの fail-fast**: クライアントのリクエストを上流へ送れない場合（例: 保存済み OAuth トークンが期限切れで refresh トークンも無い）、クライアントを自身のタイムアウトまでハングさせず、理由（`...access token expired ... run --login again`）を載せた JSON-RPC エラーを返す
